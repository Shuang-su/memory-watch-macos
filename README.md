...
异常持续时保持红色，恢复正常后自动还原。菜单可切换为 4 / 8 GiB 的宽松阈值。工具**只提醒，不自动结束进程**，不上传数据。
...
独立 Python 脚本仅依赖标准库，支持自定义阈值、PID/名称过滤和重复提醒限流；通常选择 App 或脚本其中一个运行。可对 App 内的 `Contents/MacOS/MemoryWatch` 运行 `--self-test`、`--snapshot` 或 `--test-notification` 进行诊断。
